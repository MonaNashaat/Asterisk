...
from Asterisk.heuristics_generator.models import Candidate, TemporarySpan, Sentence
from Asterisk.heuristics_generator.udf import UDF, UDFRunner
...
class CandidateExtractor(UDFRunner):
    ...
    :param candidate_class: The type of relation to extract, defined using
                            :func:`Asterisk.models.candidate_subclass <Asterisk.models.candidate.candidate_subclass>`
    :param cspaces: one or list of :class:`CandidateSpace` objects, one for each relation argument. Defines space of
                    Contexts to consider
    :param matchers: one or list of :class:`Asterisk.matchers.Matcher` objects, one for each relation argument. Only tuples of
