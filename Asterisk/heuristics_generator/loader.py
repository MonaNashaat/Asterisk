...
def split_data_tabular(X, y):
    ...
    # split dev/test
    test_ratio = 0.2
    X_tr, X_te, y_tr, y_te = sklearn.model_selection.train_test_split(X_train, y_train, test_size = test_ratio)
...
class DataLoader(object):
    ...
    def load_data_tabular (self, train_sents, test_sents, dev_sents, train_ground, test_ground, val_ground):
        ...
        for sent in train_sents:
            train_list.append(sent.text.strip().split('@'))
            
        for sent in dev_sents:
            dev_list.append(sent.text.strip().split('@'))
            
        for sent in test_sents:
            test_list.append(sent.text.strip().split('@'))
            
        df_train = pd.DataFrame(train_list)
        df_dev = pd.DataFrame(dev_list)
        test_list = pd.DataFrame(test_list)
        
        train_primitive_matrix= df_train.values
        val_primitive_matrix= df_dev.values
        test_primitive_matrix= df_test.values
